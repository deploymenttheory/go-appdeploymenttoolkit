...
func TestRunHappyPathPhases(t *testing.T) {
	...
	d := &Deployment{
		...
		PreInstall: func(ctx context.Context, s *Session) error {
			order = append(order, "pre:"+s.InstallPhase())
			return nil
		},
		Install: func(ctx context.Context, s *Session) error {
			order = append(order, "main:"+s.InstallPhase())
			return nil
		},
		PostInstall: func(ctx context.Context, s *Session) error {
	...
	assert.False(t, Active(), "session stack should be empty after Run")
}
...
func TestRunUninstallFlagDispatch(t *testing.T) {
	...
	d := &Deployment{
		Session:   testSessionOptions(t),
		Install:   func(ctx context.Context, s *Session) error { ran = "install"; return nil },
		Uninstall: func(ctx context.Context, s *Session) error { ran = "uninstall"; return nil },
	}
...
func TestRunDefaultsScriptDirectoryToExecutableDir(t *testing.T) {
	...
	opts.Hooks.Opening = append(opts.Hooks.Opening, func(ctx context.Context, s *Session) error {
