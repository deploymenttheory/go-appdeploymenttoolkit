...
func AddADTFont(ctx context.Context, opts AddADTFontOptions) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("psadt: AddADTFont: %w", err)
	}
	if strings.TrimSpace(opts.FilePath) == "" {
		return fmt.Errorf("psadt: FilePath is required: %w", ErrInvalidOption)
	}
	ext := strings.ToLower(filepath.Ext(opts.FilePath))
	suffix, ok := fontTypeSuffixes[ext]
	if !ok {
		return fmt.Errorf("psadt: file [%s] is not a supported font type: %w",
...
func RemoveADTFont(ctx context.Context, opts RemoveADTFontOptions) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("psadt: RemoveADTFont: %w", err)
	}
	if strings.TrimSpace(opts.Name) == "" {
		return fmt.Errorf("psadt: Name is required: %w", ErrInvalidOption)
	}
	fontsDir, err := systemFontsDir()
	if err != nil {
		return err
	...
	if fileName != "" {
		...
		if info, statErr := os.Stat(fontFilePath); statErr == nil && !info.IsDir() {
			...
			if err := os.Remove(fontFilePath); err != nil && !errors.Is(err, os.ErrNotExist) {
				return fmt.Errorf("psadt: removing font file: %w", err)
			}
		}
	}
	if displayName != "" {
...
func systemFontsDir() (string, error) {
	...
	if windir == "" {
		return "", winerr.Wrap("psadt: Windows directory unresolved", winerr.ErrNotFound)
	}
...
func copyFontFile(src, dest string) error {
	...
	if err != nil {
		return fmt.Errorf("psadt: reading font file: %w", err)
	}
	if err := os.WriteFile(dest, data, 0o644); err != nil { //#nosec G306 -- fonts must be world-readable
		return fmt.Errorf("psadt: writing font file: %w", err)
	}
	return nil
}
...
func sfntTableDirectoryOffset(data []byte) (uint32, error) {
	if len(data) < 12 {
		return 0, winerr.Wrap("psadt: font data is truncated", winerr.ErrInvalidOption)
	}
	if binary.BigEndian.Uint32(data) == sfntTagTTC {
		if len(data) < 16 {
			return 0, winerr.Wrap("psadt: font collection header is truncated", winerr.ErrInvalidOption)
		}
		if binary.BigEndian.Uint32(data[8:]) == 0 { // numFonts
			return 0, winerr.Wrap("psadt: font collection is empty", winerr.ErrInvalidOption)
		}
...
func sfntFindTable(data []byte, dirOffset uint32, tag string) (offset, length uint32, err error) {
	if uint64(dirOffset)+6 > uint64(len(data)) {
		return 0, 0, winerr.Wrap("psadt: font table directory out of range", winerr.ErrInvalidOption)
	}
	...
	for i := 0; i < int(numTables); i++ {
		if record+16 > uint64(len(data)) {
			return 0, 0, winerr.Wrap("psadt: font table record out of range", winerr.ErrInvalidOption)
		}
	...
	return 0, 0, winerr.Wrap("psadt: font has no name table", winerr.ErrNotFound)
}
...
func parseNameTable(data []byte, offset, length uint32) (string, error) {
	if uint64(offset)+6 > uint64(len(data)) {
		return "", winerr.Wrap("psadt: font name table out of range", winerr.ErrInvalidOption)
	}
	...
	return "", winerr.Wrap("psadt: font name not found", winerr.ErrNotFound)
}
