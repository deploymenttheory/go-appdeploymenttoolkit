...
func UpdateADTGroupPolicy(ctx context.Context) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("psadt: UpdateADTGroupPolicy: %w", err)
	}
	var errs []error
	for _, target := range []string{"Computer", "User"} {
		if err := ctx.Err(); err != nil {
			errs = append(errs, fmt.Errorf("psadt: UpdateADTGroupPolicy: %w", err))
			break
		}
		...
		if _, err := StartADTProcess(ctx, StartADTProcessOptions{
			...
			errs = append(errs, fmt.Errorf("psadt: updating Group Policy for the %s: %w", target, err))
		}
	}
