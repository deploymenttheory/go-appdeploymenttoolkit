...
func InvokeADTRegSvr32(ctx context.Context, opts InvokeADTRegSvr32Options) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("psadt: InvokeADTRegSvr32: %w", err)
	}
	...
	if !strings.EqualFold(filepath.Ext(opts.FilePath), ".dll") {
		return fmt.Errorf("psadt: FilePath %q must be a .dll file: %w", opts.FilePath, ErrInvalidOption)
	}
	if info, err := os.Stat(opts.FilePath); err != nil || info.IsDir() {
		return fmt.Errorf("psadt: DLL file [%s] does not exist: %w", opts.FilePath, ErrNotFound)
	}
...
func canonicalRegSvr32Action(action string) (string, error) {
	...
	default:
		return "", fmt.Errorf("psadt: Action %q must be Register or Unregister: %w", action, ErrInvalidOption)
	}
}
...
func resolveRegSvr32Path(arch string, osIs64, procIs64 bool, windir string) (string, error) {
	...
	default:
		return "", fmt.Errorf(
			"psadt: file has architecture [%s]; only 32-bit or 64-bit DLLs can be registered: %w",
	...
	default:
		return "", fmt.Errorf(
			"psadt: cannot register a 64-bit DLL on a 32-bit operating system: %w", ErrInvalidOption)
	}
}
