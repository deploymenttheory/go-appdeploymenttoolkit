...
func StartADTMsiProcess(ctx context.Context, opts StartADTMsiProcessOptions) (*ProcessResult, error) {
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("psadt: StartADTMsiProcess: %w", err)
	}
...
func StartADTMsiProcessAsUser(
	...
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("psadt: StartADTMsiProcessAsUser: %w", err)
	}
...
func StartADTMspProcess(ctx context.Context, opts StartADTMspProcessOptions) (*ProcessResult, error) {
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("psadt: StartADTMspProcess: %w", err)
	}
...
func StartADTMspProcessAsUser(
	...
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("psadt: StartADTMspProcessAsUser: %w", err)
	}
	msiOpts, err := opts.msiOptions()
	if err != nil {
		return nil, err
...
func (o StartADTMspProcessOptions) msiOptions() (StartADTMsiProcessOptions, error) {
	if !strings.EqualFold(filepath.Ext(o.Path), ".msp") {
		return StartADTMsiProcessOptions{}, fmt.Errorf(
			"psadt: Path %q must be an .msp file: %w", o.Path, ErrInvalidOption)
	}
...
func GetADTMsiTableProperty(
	...
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("psadt: GetADTMsiTableProperty: %w", err)
	}
	if opts.Path == "" {
		return nil, fmt.Errorf("psadt: Path is required: %w", ErrInvalidOption)
	}
...
func SetADTMsiProperty(ctx context.Context, opts SetADTMsiPropertyOptions) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("psadt: SetADTMsiProperty: %w", err)
	}
	if opts.Path == "" || opts.PropertyName == "" {
		return fmt.Errorf("psadt: Path and PropertyName are required: %w", ErrInvalidOption)
	}
...
func NewADTMsiTransform(ctx context.Context, opts NewADTMsiTransformOptions) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("psadt: NewADTMsiTransform: %w", err)
	}
	if opts.MsiPath == "" {
		return fmt.Errorf("psadt: MsiPath is required: %w", ErrInvalidOption)
	}
	if len(opts.TransformProperties) == 0 {
		return fmt.Errorf("psadt: at least one transform property is required: %w", ErrInvalidOption)
	}
...
func GetADTApplication(
	...
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("psadt: GetADTApplication: %w", err)
	}
...
func UninstallADTApplication(ctx context.Context, opts UninstallADTApplicationOptions) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("psadt: UninstallADTApplication: %w", err)
	}
	if len(opts.Name) == 0 && len(opts.ProductCode) == 0 && opts.FilterScript == nil {
		return fmt.Errorf("psadt: Name, ProductCode or FilterScript is required: %w", ErrInvalidOption)
	}
	...
	for _, app := range apps {
		if err := ctx.Err(); err != nil {
			errs = append(errs, fmt.Errorf("psadt: UninstallADTApplication: %w", err))
			break
		}
		var err error
		...
		if err != nil {
			errs = append(errs, fmt.Errorf("psadt: uninstalling [%s]: %w", app.DisplayName, err))
		}
	}
...
func prepareMsiProcess(ctx context.Context, opts *StartADTMsiProcessOptions) (*msiPreparation, error) {
	...
	if action == "Install" && msipkg.IsProductCode(opts.Path) {
		return nil, fmt.Errorf(
			"psadt: a product code can only be used with non-install actions: %w", ErrInvalidOption)
	}
...
func canonicalMsiAction(action string) (string, error) {
	...
	return "", fmt.Errorf("psadt: Action %q: %w", action, ErrInvalidOption)
}
...
func resolveMsiProduct(s *DeploymentSession, path string) (string, error) {
	...
	return "", fmt.Errorf("psadt: failed to find the file [%s]: %w", path, ErrNotFound)
}
...
func buildMsiArguments(in msiArgumentInputs, msiCfg config.MSI) (string, error) {
	...
	default:
		return "", fmt.Errorf("psadt: Action %q: %w", in.Action, ErrInvalidOption)
	}
