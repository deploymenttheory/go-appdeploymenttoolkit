...
func userProfiles(_ context.Context, opts GetADTUserProfilesOptions) ([]UserProfile, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("psadt: opening ProfileList: %w", err)
	}
	defer k.Close()
	sids, err := k.ReadSubKeyNames(-1)
	if err != nil {
		return nil, fmt.Errorf("psadt: enumerating ProfileList: %w", err)
	}
...
func convertAccountOrSID(accountOrSID string) (string, error) {
	if strings.HasPrefix(strings.ToUpper(accountOrSID), "S-1-") {
		...
		if err != nil {
			return "", fmt.Errorf("psadt: parsing SID %s: %w", accountOrSID, err)
		}
		user, domain, _, err := sid.LookupAccount("")
		if err != nil {
			return "", fmt.Errorf("psadt: resolving SID %s: %w", accountOrSID, err)
		}
		if domain == "" {
			return user, nil
		}
	...
	if err != nil {
		return "", fmt.Errorf("psadt: resolving account %s: %w", accountOrSID, err)
	}
	out, err := sid.String(), error(nil)
	if out == "" {
		return "", winerr.Wrap("psadt: SID for "+accountOrSID, winerr.ErrNotFound)
	}
	return out, err
}
