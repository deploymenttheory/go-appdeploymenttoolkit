// Package psadt: system-state probes (Phase 5).
//
...
func TestADTBattery(ctx context.Context) (BatteryInfo, error) {
	if err := ctx.Err(); err != nil {
		return BatteryInfo{}, fmt.Errorf("psadt: %w", err)
	}
...
func GetADTPendingReboot(ctx context.Context) (PendingRebootInfo, error) {
	if err := ctx.Err(); err != nil {
		return PendingRebootInfo{}, fmt.Errorf("psadt: %w", err)
	}
...
func GetADTUserToastNotificationMode(ctx context.Context) (ToastNotificationMode, bool, error) {
	if err := ctx.Err(); err != nil {
		return ToastNotificationModeUnrestricted, false, fmt.Errorf("psadt: %w", err)
	}
...
func anyNetworkConnection() (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("psadt: enumerating network interfaces: %w", err)
	}
	for i := range ifaces {
		iface := ifaces[i]
