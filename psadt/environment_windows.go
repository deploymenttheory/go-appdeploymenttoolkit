...
func environmentKey(target EnvironmentVariableTarget, access uint32) (registry.Key, error) {
	if target == EnvironmentTargetMachine {
		...
		if err != nil {
			return 0, fmt.Errorf("psadt: opening machine environment key: %w", err)
		}
		return k, nil
	}
	k, err := registry.OpenKey(registry.CURRENT_USER, `Environment`, access)
	if err != nil {
		return 0, fmt.Errorf("psadt: opening user environment key: %w", err)
	}
	return k, nil
}
...
func getEnvironmentVariableFromRegistry(
	...
	if err != nil {
		if errors.Is(err, registry.ErrNotExist) {
			return "", fmt.Errorf("psadt: environment variable %s (%s): %w",
				variable, target, winerr.ErrNotFound)
		}
		return "", fmt.Errorf("psadt: reading environment variable %s (%s): %w",
			variable, target, err)
	}
	return value, nil
}
...
func setEnvironmentVariableInRegistry(
	...
	if err != nil {
		return fmt.Errorf("psadt: writing environment variable %s (%s): %w", variable, target, err)
	}
...
func removeEnvironmentVariableFromRegistry(
	...
	if err := k.DeleteValue(variable); err != nil {
		...
		return fmt.Errorf("psadt: deleting environment variable %s (%s): %w", variable, target, err)
	}
	broadcastEnvironmentChange()
	return nil
}
