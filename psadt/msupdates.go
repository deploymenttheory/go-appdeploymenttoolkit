...
func InstallADTMSUpdates(ctx context.Context, opts InstallADTMSUpdatesOptions) error {
	...
	if len(updates) == 0 {
		return fmt.Errorf("psadt: no .msu updates found in %s: %w", opts.Directory, winerr.ErrNotFound)
	}
...
func discoverMSUpdates(path string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("psadt: reading update path: %w", err)
	}
	if !info.IsDir() {
		if !strings.EqualFold(filepath.Ext(path), ".msu") {
			return nil, winerr.Wrap("psadt: update path is not a .msu file", winerr.ErrInvalidOption)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("psadt: scanning for updates: %w", err)
	}
	return updates, nil
}
