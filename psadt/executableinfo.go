...
func GetADTExecutableInfo(ctx context.Context, path string) (ExecutableInfo, error) {
	if err := ctx.Err(); err != nil {
		return ExecutableInfo{}, fmt.Errorf("psadt: GetADTExecutableInfo: %w", err)
	}
	if info, err := os.Stat(path); err != nil || info.IsDir() {
		return ExecutableInfo{}, fmt.Errorf("psadt: file [%s] does not exist: %w", path, ErrNotFound)
	}
...
func peImageSubsystem(path string) (uint16, error) {
	f, err := pe.Open(path)
	if err != nil {
		return 0, fmt.Errorf("psadt: parsing PE file [%s]: %w", path, err)
	}
	...
	case *pe.OptionalHeader64:
		return oh.Subsystem, nil
	default:
		return 0, winerr.Wrap("psadt: file has no PE optional header", winerr.ErrInvalidOption)
	}
}
