...
func ShowADTInstallationWelcome(
	...
	if err := ctx.Err(); err != nil {
		return WelcomeResult{}, fmt.Errorf("psadt: %w", err)
	}
	s, err := GetADTSession()
	if err != nil {
	...
	for {
		...
		case dlgButtonDefer:
			...
			return WelcomeResult{Action: dlgButtonDefer, Deferred: true},
				fmt.Errorf("psadt: welcome deferred: %w", ErrDeferred)
...
func SendADTKeys(ctx context.Context, opts SendADTKeysOptions) error {
	...
	if opts.WaitSeconds > 0 {
		select {
		case <-ctx.Done():
			return fmt.Errorf("psadt: %w", ctx.Err())
...
func BlockADTAppExecution(ctx context.Context, processNames []string) error {
	...
	if len(processNames) == 0 {
		return winerr.Wrap("psadt: at least one process name is required", winerr.ErrInvalidOption)
	}
