...
func ConvertADTRegistryPath(key string, wow6432Node bool) (string, error) {
	if strings.TrimSpace(key) == "" {
		return "", winerr.Wrap("psadt: registry key path is empty", winerr.ErrInvalidOption)
	}
	...
	if !ok {
		return "", winerr.Wrap("psadt: unable to detect target registry hive in ["+key+"]", winerr.ErrInvalidOption)
	}
...
func GetADTRegistryKey(ctx context.Context, opts GetADTRegistryKeyOptions) (any, error) {
	...
	if !exists {
		...
		return nil, winerr.Wrap("psadt: registry key "+canonical, winerr.ErrNotFound)
	}
	if opts.Name != "" {
		logToSession(
	...
	if len(values) == 0 && !opts.ReturnEmptyKeyIfExists {
		...
		return nil, winerr.Wrap("psadt: registry key "+canonical+" has no values", winerr.ErrNotFound)
	}
...
func coerceRegistryValue(value any, kind RegistryValueKind) (regkey.Value, error) {
	...
	case RegistryValueKindString, RegistryValueKindExpandString:
		s, ok := value.(string)
		if !ok {
			return regkey.Value{}, winerr.Wrap("psadt: registry string value requires a Go string", winerr.ErrInvalidOption)
		}
		k := regkey.KindString
	...
	case RegistryValueKindMultiString:
		...
		if !ok {
			return regkey.Value{}, winerr.Wrap(
				"psadt: registry multi-string value requires a Go []string",
	...
	case RegistryValueKindBinary:
		b, ok := value.([]byte)
		if !ok {
			return regkey.Value{}, winerr.Wrap("psadt: registry binary value requires a Go []byte", winerr.ErrInvalidOption)
		}
	...
	case RegistryValueKindDWord:
		...
		if u > math.MaxUint32 {
			return regkey.Value{}, winerr.Wrap("psadt: registry DWord value overflows 32 bits", winerr.ErrInvalidOption)
		}
		return regkey.Value{
	...
	default:
		return regkey.Value{}, winerr.Wrap("psadt: unsupported registry value type", winerr.ErrInvalidOption)
	}
}
...
func toUint64(value any) (uint64, error) {
	...
	case uint64:
		return v, nil
	default:
		return 0, winerr.Wrap("psadt: registry integer value requires a Go integer type", winerr.ErrInvalidOption)
	}
}
...
func RemoveADTRegistryKey(ctx context.Context, opts RemoveADTRegistryKeyOptions) error {
	...
	if opts.Name == "" {
		if !opts.Recurse {
			...
			if len(subs) > 0 {
				return winerr.Wrap(
					"psadt: unable to delete child key(s) of ["+canonical+"] without Recurse",
					winerr.ErrInvalidOption,
				)
			}
		}
...
func InvokeADTAllUsersRegistryAction(
	...
	if action == nil {
		return winerr.Wrap("psadt: all-users registry action is nil", winerr.ErrInvalidOption)
	}
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("psadt: %w", err)
	}
	...
	for _, sid := range sids {
		...
		if err := ctx.Err(); err != nil {
			errs = append(errs, fmt.Errorf("psadt: %w", err))
			break
		}
		logToSession(
		...
		if err := action(ctx, sid); err != nil {
			...
			errs = append(errs, fmt.Errorf("psadt: all-users registry action for %s: %w", sid, err))
		}
	}
