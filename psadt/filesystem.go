...
var errFolderNotEmpty = errors.New("psadt: folder not empty")
...
func removeOne(item string, recurse bool) error {
	...
	if err != nil {
		return fmt.Errorf("psadt: reading %s: %w", item, err)
	}
	if fi.IsDir() {
		if !recurse {
			logToSession(
		...
		if err := os.RemoveAll(item); err != nil {
			return fmt.Errorf("psadt: deleting %s: %w", item, err)
		}
		return nil
	}
...
func NewADTFolder(ctx context.Context, path string) error {
	...
	if err := os.MkdirAll(path, 0o755); err != nil { //#nosec G301 -- parity with New-Item defaults
		return fmt.Errorf("psadt: creating folder %s: %w", path, err)
	}
	return nil
}
...
func RemoveADTFolder(ctx context.Context, opts RemoveADTFolderOptions) error {
	...
	if !opts.DisableRecursion {
		...
		if err := os.RemoveAll(opts.Path); err != nil {
			return fmt.Errorf("psadt: deleting folder %s: %w", opts.Path, err)
		}
		return nil
	}
	logToSession(
...
func removeFolderNonRecursive(path string) error {
	...
	if err != nil {
		return fmt.Errorf("psadt: reading folder %s: %w", path, err)
	}
	var skipped []string
	for _, entry := range entries {
		...
		if err := os.Remove(child); err != nil {
			return fmt.Errorf("psadt: deleting %s: %w", child, err)
		}
	}
	if len(skipped) > 0 {
		return fmt.Errorf("psadt: the following subfolders are not empty ['%s']: %w",
			strings.Join(skipped, "'; '"), errFolderNotEmpty)
	}
	if err := os.Remove(path); err != nil {
		return fmt.Errorf("psadt: deleting folder %s: %w", path, err)
	}
	return nil
}
...
// delegates to GetADTUserProfiles (psadt/users.go); it is a variable so
...
func CopyADTContentToCache(ctx context.Context, path string) (string, error) {
	...
	if fi, err := os.Stat(scriptDir); err != nil || !fi.IsDir() {
		return "", fmt.Errorf("psadt: script directory %s: %w", scriptDir, winerr.ErrNotFound)
	}
	cachePath := path
	if cachePath == "" {
	...
	if err := os.MkdirAll(cachePath, 0o755); err != nil { //#nosec G301 -- parity with New-Item defaults
		return "", fmt.Errorf("psadt: creating cache folder %s: %w", cachePath, err)
	}
...
func RemoveADTContentFromCache(ctx context.Context) error {
	...
	if err := os.RemoveAll(cachePath); err != nil {
		return fmt.Errorf("psadt: removing cache folder %s: %w", cachePath, err)
	}
	return nil
}
...
func NewADTZipFile(ctx context.Context, opts NewADTZipFileOptions) error {
	...
	if _, err := os.Stat(opts.DestinationPath); err == nil {
		if !opts.Overwrite {
			return fmt.Errorf("psadt: archive %s already exists: %w", opts.DestinationPath, fs.ErrExist)
		}
		logToSession(
		...
		if err := os.Remove(opts.DestinationPath); err != nil {
			return fmt.Errorf("psadt: deleting existing archive %s: %w", opts.DestinationPath, err)
		}
	}
	logToSession(
	...
	if opts.RemoveSourceAfterArchiving {
		for _, src := range opts.LiteralPath {
			...
			if err := os.RemoveAll(src); err != nil {
				return fmt.Errorf("psadt: deleting archived source %s: %w", src, err)
			}
		}
	}
	return nil
...
func writeZipArchive(sources []string, dest string) error {
	...
	if err != nil {
		return fmt.Errorf("psadt: creating archive %s: %w", dest, err)
	}
	...
	if err := zw.Close(); err != nil {
		_ = out.Close()
		return fmt.Errorf("psadt: finalizing archive %s: %w", dest, err)
	}
	if err := out.Close(); err != nil {
		return fmt.Errorf("psadt: closing archive %s: %w", dest, err)
	}
	return nil
}
...
func addZipEntry(zw *zip.Writer, src string) error {
	...
	if err != nil {
		return fmt.Errorf("psadt: archive source %s: %w", src, winerr.ErrNotFound)
	}
	if !fi.IsDir() {
	...
	return filepath.WalkDir(src, func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return fmt.Errorf("psadt: walking %s: %w", path, err)
		}
		if d.IsDir() {
			return nil
		}
		...
		if err != nil {
			return fmt.Errorf("psadt: relativizing %s: %w", path, err)
		}
		info, err := d.Info()
		if err != nil {
			return fmt.Errorf("psadt: reading %s: %w", path, err)
		}
...
func addZipFile(zw *zip.Writer, path, name string, fi fs.FileInfo) error {
	...
	if err != nil {
		return fmt.Errorf("psadt: archiving %s: %w", path, err)
	}
	hdr.Name = name
	...
	if err != nil {
		return fmt.Errorf("psadt: opening %s: %w", path, err)
	}
...
func GetADTFileVersionInfo(ctx context.Context, path string) (fsops.FileVersionInfo, error) {
	if err := ctx.Err(); err != nil {
		return fsops.FileVersionInfo{}, fmt.Errorf("psadt: %w", err)
	}
...
func GetADTFreeDiskSpace(ctx context.Context, drive string) (uint64, error) {
	if err := ctx.Err(); err != nil {
		return 0, fmt.Errorf("psadt: %w", err)
	}
	if drive == "" {
