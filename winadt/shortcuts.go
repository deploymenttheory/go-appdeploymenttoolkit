...
func NewADTShortcut(ctx context.Context, s Shortcut) error {
	...
	if err := os.MkdirAll(filepath.Dir(s.Path), 0o755); err != nil {
		return fmt.Errorf("adt: creating shortcut directory: %w", err)
	}
...
func RemoveADTDesktopShortcut(ctx context.Context, name string) error {
	...
	if desktop == "" {
		return winerr.Wrap("adt: common desktop path unresolved", winerr.ErrNotFound)
	}
	target := filepath.Join(desktop, "Desktop", name)
	err := os.Remove(target)
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("adt: removing desktop shortcut: %w", err)
	}
