...
func AddADTEdgeExtension(ctx context.Context, opts AddADTEdgeExtensionOptions) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("adt: AddADTEdgeExtension: %w", err)
	}
	if strings.TrimSpace(opts.ExtensionID) == "" {
		return fmt.Errorf("adt: ExtensionID is required: %w", ErrInvalidOption)
	}
	if _, ok := edgeInstallationModes[opts.InstallationMode]; !ok {
		return fmt.Errorf("adt: InstallationMode %q is not valid: %w", opts.InstallationMode, ErrInvalidOption)
	}
	if !isWellFormedAbsoluteURL(opts.UpdateURL) {
		return fmt.Errorf("adt: UpdateUrl %q is not a valid URL: %w", opts.UpdateURL, ErrInvalidOption)
	}
...
func RemoveADTEdgeExtension(ctx context.Context, opts RemoveADTEdgeExtensionOptions) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("adt: RemoveADTEdgeExtension: %w", err)
	}
	if strings.TrimSpace(opts.ExtensionID) == "" {
		return fmt.Errorf("adt: ExtensionID is required: %w", ErrInvalidOption)
	}
...
func parseEdgeExtensions(data string) (map[string]edgeExtension, error) {
	...
	if err := json.Unmarshal([]byte(trimmed), &out); err != nil {
		return nil, fmt.Errorf("adt: decoding Edge ExtensionSettings: %w", err)
	}
	return out, nil
}
...
func marshalEdgeExtensions(exts map[string]edgeExtension) (string, error) {
	...
	for i, id := range ids {
		...
		if err != nil {
			return "", fmt.Errorf("adt: encoding Edge extension id: %w", err)
		}
		value, err := json.Marshal(exts[id])
		if err != nil {
			return "", fmt.Errorf("adt: encoding Edge extension: %w", err)
		}
		b.Write(key)
		b.WriteByte(':')
		b.Write(value)
