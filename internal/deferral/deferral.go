...
func parseRoundTrip(data any) (time.Time, error) {
	s, ok := data.(string)
	if !ok {
		return time.Time{}, winerr.Wrap("deferral: timestamp type", winerr.ErrInvalidOption)
	}
	...
	return time.Time{}, winerr.Wrap("deferral: parsing timestamp "+s, winerr.ErrInvalidOption)
}
...
func parseTimeSpan(data any) (time.Duration, error) {
	s, ok := data.(string)
	if !ok {
		return 0, winerr.Wrap("deferral: timespan type", winerr.ErrInvalidOption)
	}
	...
	if dpart, rest, ok := strings.Cut(
		...
		if _, err := fmt.Sscanf(dpart, "%d", &days); err != nil {
			return 0, winerr.Wrap("deferral: parsing timespan days "+s, winerr.ErrInvalidOption)
		}
		s = rest
	}
	var h, m int64
	...
	if _, err := fmt.Sscanf(s, "%d:%d:%f", &h, &m, &sec); err != nil {
		return 0, winerr.Wrap("deferral: parsing timespan "+s, winerr.ErrInvalidOption)
	}
