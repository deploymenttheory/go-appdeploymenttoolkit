...
// winerr.ErrNotFound when the file, section or key does not exist.
func ReadValue(path, section, key string) (string, error) {
	doc, err := load(path)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return "", winerr.Wrap("inifile: file "+path, winerr.ErrNotFound)
		}
		return "", err
	}
	header, end, found := sectionRange(doc.lines, section)
	if !found {
		return "", winerr.Wrap("inifile: section "+section, winerr.ErrNotFound)
	}
	value, matched := "", false
	...
	if !matched {
		return "", winerr.Wrap("inifile: key "+key, winerr.ErrNotFound)
	}
	return unquote(value), nil
}

// ReadSection returns all key=value pairs of section (last-wins for
// duplicate keys). The error wraps winerr.ErrNotFound when the file or the
// section does not exist.
func ReadSection(path, section string) (map[string]string, error) {
	doc, err := load(path)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return nil, winerr.Wrap("inifile: file "+path, winerr.ErrNotFound)
		}
		return nil, err
	}
	header, end, found := sectionRange(doc.lines, section)
	if !found {
		return nil, winerr.Wrap("inifile: section "+section, winerr.ErrNotFound)
	}
...
// appearance. The error wraps winerr.ErrNotFound when the file is missing.
