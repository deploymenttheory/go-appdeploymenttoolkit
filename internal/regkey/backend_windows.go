...
func rootKey(hive string) (registry.Key, error) {
	...
	default:
		return 0, winerr.Wrap("regkey: hive "+hive, winerr.ErrInvalidOption)
	}
}

func mapNotFound(op string, err error) error {
	if errors.Is(err, registry.ErrNotExist) {
		return fmt.Errorf("%s: %w", op, winerr.ErrNotFound)
	}
...
func (n *Native) SetValue(hive, path, name string, value Value) error {
	...
	default:
		return winerr.Wrap("regkey: value kind", winerr.ErrInvalidOption)
	}
	if err != nil {
