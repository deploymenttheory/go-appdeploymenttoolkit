...
func (f *Fake) GetValue(hive, path, name string) (Value, error) {
	...
	if !ok {
		return Value{}, errs.Wrap("fake: key "+path, errs.ErrNotFound)
	}
	v, ok := vals[name]
	if !ok {
		return Value{}, errs.Wrap("fake: value "+name, errs.ErrNotFound)
	}
	return v, nil
}
...
func (f *Fake) DeleteValue(hive, path, name string) error {
	...
	vals, ok := f.keys[fkey(hive, path)]
	if !ok {
		return errs.Wrap("fake: key "+path, errs.ErrNotFound)
	}
	if _, ok := vals[name]; !ok {
		return errs.Wrap("fake: value "+name, errs.ErrNotFound)
	}
	delete(vals, name)
	return nil
}
...
func (f *Fake) EnumValues(hive, path string) (map[string]Value, error) {
	...
	vals, ok := f.keys[fkey(hive, path)]
	if !ok {
		return nil, errs.Wrap("fake: key "+path, errs.ErrNotFound)
	}
