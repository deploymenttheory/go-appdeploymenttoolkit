...
type Backend interface {
	// GetValue returns the named value; errs.ErrNotFound when the key or
	...
	// DeleteValue removes a value; errs.ErrNotFound if absent.
...
func SplitRoot(path string) (hive, subkey string, err error) {
	...
	case "HKCC", "HKEY_CURRENT_CONFIG":
		hive = "HKCC"
	default:
		return "", "", errs.Wrap("regkey: unrecognized hive "+root, errs.ErrInvalidOption)
	}
