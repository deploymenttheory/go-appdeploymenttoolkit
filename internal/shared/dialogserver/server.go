...
func errClosed() error {
	return errs.Wrap("dialogserver: server is closed", errs.ErrDialogUnavailable)
}
...
func (r *pipeRenderer) do(
	...
	if err := ctx.Err(); err != nil {
		return nil, errs.Wrap("dialogserver", err)
	}
...
func (r *pipeRenderer) ShowModal(
	...
	if len(raw) > 0 {
		if err := json.Unmarshal(raw, &res); err != nil {
			return ipc.ModalDialogResult{}, errs.Wrap(
				"dialogserver: decoding modal result", errs.ErrDialogUnavailable)
		}
	}
	return res, nil
}
...
func (r *pipeRenderer) PromptToCloseApps(
	...
	if len(raw) > 0 {
		if err := json.Unmarshal(raw, &res); err != nil {
			return ipc.PromptToCloseAppsResult{}, errs.Wrap(
				"dialogserver: decoding prompt-to-close result", errs.ErrDialogUnavailable)
		}
	}
	return res, nil
}
...
func (r *pipeRenderer) GetWindowInfo(
	...
	if len(raw) > 0 {
		if err := json.Unmarshal(raw, &res); err != nil {
			return ipc.WindowInfoResult{}, errs.Wrap(
				"dialogserver: decoding window info", errs.ErrDialogUnavailable)
		}
	}
	return res, nil
}
