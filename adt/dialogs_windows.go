...
func newDialogServer(ctx context.Context, _ *DeploymentSession) (*dialogserver.DialogServer, error) {
	...
	if !ok {
		return nil, winerr.Wrap("adt: no interactive user session", winerr.ErrDialogUnavailable)
	}
	if target.IsCurrent {
...
func initiateSystemRestart(_ context.Context, delaySeconds int, message string) error {
	...
	if err := shutdown.InitiateSystemShutdownEx(
		...
		return winerr.Wrap("adt: InitiateSystemShutdownEx", err)
	}
	return nil
}
...
func blockAppExecution(ctx context.Context, s *DeploymentSession, names []string) error {
	if !s.IsAdmin() {
		return winerr.Wrap("adt: blocking application execution requires administrator rights", winerr.ErrInvalidOption)
	}
	exe, err := os.Executable()
	if err != nil {
		return fmt.Errorf("adt: resolving executable: %w", err)
	}
...
func unblockAppExecution(ctx context.Context, s *DeploymentSession) error {
	if !s.IsAdmin() {
		return winerr.Wrap("adt: unblocking application execution requires administrator rights", winerr.ErrInvalidOption)
	}
	base, err := registry.OpenKey(registry.LOCAL_MACHINE, ifeoPath, registry.ENUMERATE_SUB_KEYS)
	if err != nil {
		return fmt.Errorf("adt: opening IFEO key: %w", err)
	}
	subs, err := base.ReadSubKeyNames(-1)
	_ = base.Close()
	if err != nil {
		return fmt.Errorf("adt: enumerating IFEO subkeys: %w", err)
	}
...
func registerUnblockTask(exe, taskName string) error {
	...
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("adt: schtasks create: %w", err)
	}
	return nil
}
...
func unregisterUnblockTask(taskName string) error {
	...
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("adt: schtasks delete: %w", err)
	}
	return nil
}
...
func queryUserNotificationState() (int, error) {
	...
	if err := shell.SHQueryUserNotificationState(&state); err != nil {
		return 0, winerr.Wrap("adt: SHQueryUserNotificationState", err)
	}
