...
func MountADTWimFile(ctx context.Context, opts MountADTWimFileOptions) (*MountedWimFile, error) {
	if err := ctx.Err(); err != nil {
		return nil, fmt.Errorf("adt: MountADTWimFile: %w", err)
	}
	if err := validateMountOptions(opts); err != nil {
		return nil, err
	}
	if info, err := os.Stat(opts.ImagePath); err != nil || info.IsDir() {
		return nil, fmt.Errorf("adt: image path [%s] cannot be found: %w", opts.ImagePath, ErrNotFound)
	}
...
func DismountADTWimFile(ctx context.Context, opts DismountADTWimFileOptions) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("adt: DismountADTWimFile: %w", err)
	}
	if strings.TrimSpace(opts.Path) == "" {
		return fmt.Errorf("adt: Path is required: %w", ErrInvalidOption)
	}
	...
	if err := os.Remove(opts.Path); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("adt: removing mount directory: %w", err)
	}
	return nil
}
...
func validateMountOptions(opts MountADTWimFileOptions) error {
	if strings.TrimSpace(opts.ImagePath) == "" {
		return fmt.Errorf("adt: ImagePath is required: %w", ErrInvalidOption)
	}
	if strings.TrimSpace(opts.Path) == "" {
		return fmt.Errorf("adt: Path is required: %w", ErrInvalidOption)
	}
	...
	case opts.Index > 0 && hasName:
		return fmt.Errorf("adt: specify either Index or Name, not both: %w", ErrInvalidOption)
	case opts.Index <= 0 && !hasName:
		return fmt.Errorf("adt: an image Index or Name is required: %w", ErrInvalidOption)
	case opts.Index < 0:
		return fmt.Errorf("adt: Index must be a positive image index: %w", ErrInvalidOption)
	}
	return nil
}
...
func prepareMountDir(path string, force bool) error {
	...
	case err == nil && len(entries) > 0:
		if !force {
			return fmt.Errorf("adt: mount path [%s] is not empty: %w", path, ErrInvalidOption)
		}
		if err := os.RemoveAll(path); err != nil {
			return fmt.Errorf("adt: removing pre-existing mount path: %w", err)
		}
	case err != nil && !os.IsNotExist(err):
		return fmt.Errorf("adt: inspecting mount path: %w", err)
	}
	if err := os.MkdirAll(path, 0o755); err != nil {
		return fmt.Errorf("adt: creating mount path: %w", err)
	}
	return nil
}
